...
const page = () => {
  ...
  const fetchQCProcessData = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${Domain}/po/grn-qc`, {
        headers: {
          Authorization: token,
        },
      });
      ...
      setInventoryEntries(data.inventoryItems);
      setFilteredEntries(data.inventoryItems);
    } catch (error) {
  ...
  const handleSubmit = async (inventoryId) => {
    ...
    try {
      ...
      if (selectedStateName === "rejected" || selectedStateName === "hold") {
        ...
        const response = await fetch(`${Domain}/po/perform-qc`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: token,
    ...
    <>
      ...
      <div className="lg:p-8 p-4 bg-black">
        <Card className="p-4 bg-gray-900">
          ...
          {selectedTab === "rejected" && filteredEntries.length>0
